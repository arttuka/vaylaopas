    ...
    await Promise.all(
      lanes.map(
        async (lane): Promise<void> => {
          ...
          await Promise.all(
            lane.coordinates.map(
              (c, i): Promise<QueryResult> =>
                client.query(
...
export const saveIntersections = async (
  ...
  try {
    ...
    await Promise.all(
      intersections.map(
        async (is): Promise<void> => {
          await client.query(
            'INSERT INTO intersection(id, x, y) VALUES ($1, $2, $3)',
            [is.id, is.x, is.y]
          )
          await Promise.all(
            Array.from(is.lanes).map(
              (laneId): Promise<QueryResult> =>
                client.query(
...
export const getLanes = async (): Promise<Lane[]> => {
  ...
  const coordinates: Dictionary<Coordinate[]> = groupBy(
    (await client.query(
      'SELECT * FROM coordinate ORDER BY lane ASC, index ASC'
    )).rows,
    'lane'
  )
  const lanes = (await client.query('SELECT * FROM lane')).rows.map(
    (lane): Lane => ({
      ...lane,
...
export const getIntersections = async (): Promise<Intersection[]> => {
  const client = await pool.connect()
  const intersectionLanes: Dictionary<
    { intersection: number; lane: number }[]
  > = groupBy(
